...
public class Data_Controller : MonoBehaviour
{
    ...
    private void CollectSaveableEntities()
    {
        ...
        GameObject player = GameObject.Find("Character_PC_Quest");
        if (player != null)
        {
            DataActor.targetObjects.Add(player);
        }
    ...
    public void ResetDataAndRestartScene()
    {
        if (DataActor != null)
        {
            DataActor.Act_ResetAndClearData(); // 세이브 파일 삭제
            SceneManager.LoadScene(SceneManager.GetActiveScene().name); // 씬 처음부터 재시작
        }
    }
}