...
public class Actor_Scene : MonoBehaviour
{
    ...
    // [추가] 초기화 기둥으로 사용할지 여부를 결정하는 체크박스
    [Header("초기화 옵션")]
    [Tooltip("체크하면 데이터를 저장하지 않고, 세이브 파일을 완전히 지운 뒤 씬을 로드합니다.")]
    public bool isResetScene = false;
    ...
    public void Act_LoadScene()
    {
        ...
        print($"{SceneName}으로 이전을 준비합니다. (초기화 모드: {isResetScene})");
    ...
    private IEnumerator SafeSceneTransitionSequence()
    {
        ...
        {
            // [수정] 체크박스가 켜져 있다면 저장 대신 파일 삭제(초기화)를 호출합니다.
            if (isResetScene)
            {
                if (Data_Controller.Instance.DataActor != null)
                {
